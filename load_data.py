...
def get_data_Cross_Label(data_path, task_index, dataset):
    if dataset == 'MSCOCO' or dataset == 'MSCOCO_NoMean':
        test_image = torch.from_numpy(np.swapaxes(h5py.File(data_path + "/test_{}_image.mat".format(task_index), 'r')['test_{}_image'.format(task_index)], 0, 1))
        test_text = torch.from_numpy(np.swapaxes(h5py.File(data_path + "/test_{}_text.mat".format(task_index), 'r')['test_{}_text'.format(task_index)], 0, 1))
        test_label = torch.from_numpy(np.swapaxes(h5py.File(data_path + "/test_{}_label.mat".format(task_index), 'r')['test_{}_lab'.format(task_index)], 0, 1))
        ...
        test_image = torch.from_numpy(np.swapaxes(h5py.File(data_path + "/test_task_{}_image.mat".format(task_index), 'r')['test_{}_image'.format(task_index)], 0, 1))
        test_text = torch.from_numpy(np.swapaxes(h5py.File(data_path + "/test_task_{}_text.mat".format(task_index), 'r')['test_{}_text'.format(task_index)], 0, 1))
        test_label = torch.from_numpy(np.swapaxes(h5py.File(data_path + "/test_task_{}_label.mat".format(task_index), 'r')['test_{}_label'.format(task_index)], 0, 1))

        database_image = torch.from_numpy(np.swapaxes(h5py.File(data_path + "/database_task_{}_image.mat".format(task_index), 'r')['database_{}_image'.format(task_index)], 0, 1))
        database_text = torch.from_numpy(np.swapaxes(h5py.File(data_path + "/database_task_{}_text.mat".format(task_index), 'r')['database_{}_text'.format(task_index)], 0, 1))
        database_label = torch.from_numpy(np.swapaxes(h5py.File(data_path + "/database_task_{}_label.mat".format(task_index), 'r')['database_{}_label'.format(task_index)], 0, 1))
    return test_image, test_text, test_label, database_image, database_text, database_label


def get_loader(args, task_index):
    train_image, train_text, train_label, test_image, test_text, test_label, database_image, database_text, database_label = get_data(args.dataset_path, task_index, args.dataset_name)
    ...
    num_database = database_image.shape[0]
    ...
    input_data_par['number_train'] = num_train
    input_data_par['num_class'] = num_class
    input_data_par['num_database'] = num_database
